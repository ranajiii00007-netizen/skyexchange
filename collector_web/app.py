...
def admin_wallet():
    ...
    query = f"""
        SELECT id, transaction_date, type, amount, currency, description, added_by, created_at
    ...
    cur.execute(query, params)
    wallet_entries = [dict(
        id=r[0], transaction_date=r[1], type=r[2], amount=r[3], currency=r[4],
        description=r[5], added_by=r[6], created_at=r[7]
    ) for r in cur.fetchall()]
    ...
    for t_type, curr, amt in totals_by_type:
        amt = float(amt or 0)
        curr = (curr or "EUR").upper()
        if curr not in currency_balances:
            currency_balances[curr] = 0.0

        if t_type in ("DEPOSIT", "PROFIT"):
            currency_balances[curr] += amt
            total_deposits += amt
    ...
    net_balance = total_deposits + total_adjustments - total_withdrawals
    conn.close()
    ...
    filters = {"type": w_type, "currency": currency, "date_from": date_from, "date_to": date_to}

    return render_template(
        "admin_wallet.html", active_page="wallet",
        dropdowns=dropdowns, filters=filters, totals=totals,
        currency_balances=currency_balances, wallet_entries=wallet_entries,
        today=str(date.today())
    )


@app.route("/admin/wallet/save", methods=["POST"])
@admin_required
def admin_wallet_save():
    w_type = request.form.get("type", "DEPOSIT").strip().upper()
    amount_val = safe_float(request.form.get("amount", "0"))
    currency = request.form.get("currency", "EUR").strip().upper()
    tx_date = request.form.get("transaction_date", str(date.today())).strip()
    description = request.form.get("description", "").strip()
    added_by = session.get("collector_name") or session.get("user") or "Admin"
    ...
    if amount_val <= 0:
        ...
        try:
            cur.execute("""
                INSERT INTO wallet_transactions (transaction_date, type, amount, currency, description, added_by, created_at)
                VALUES (?, ?, ?, ?, ?, ?, ?)
            """, (tx_date, w_type, amount_val, currency, description, added_by, datetime.now().isoformat()))
            conn.commit()
            database.bump_app_revision()
            flash("Wallet record saved successfully.", "success")
        except Exception as exc:
            conn.rollback()
            flash(f"Error saving wallet record: {exc}", "error")
        finally:
            conn.close()

    redirect_args = {k: request.args.get(k, "").strip() for k in ("type", "currency", "date_from", "date_to") if request.args.get(k, "").strip()}
    return redirect(url_for("admin_wallet", **redirect_args))


@app.route("/admin/wallet/delete/<int:entry_id>", methods=["POST"])
@admin_required
def admin_wallet_delete(entry_id):
    conn = db()
    cur = conn.cursor()
    try:
        cur.execute("DELETE FROM wallet_transactions WHERE id=?", (entry_id,))
        conn.commit()
        database.bump_app_revision()
        flash("Wallet entry deleted successfully.", "success")
    except Exception as exc:
        conn.rollback()
        flash(f"Error deleting wallet entry: {exc}", "error")
    finally:
        conn.close()
...
def admin_payments():
    ...
    cur.execute("SELECT name FROM bankers WHERE status=1 ORDER BY name")
    bankers = [dict(name=row[0]) for row in cur.fetchall()]
    dropdowns = {"customers": customers, "collectors": collectors, "currencies": currencies, "bankers": bankers}
    ...
    if party_name:
        where_clauses.append("LOWER(party_name) LIKE ?")
    ...
    where_sql = (" WHERE " + " AND ".join(where_clauses)) if where_clauses else ""
    query = f"""
        SELECT id, payment_date, direction, party_type, party_name, amount, currency, payment_method, reference_no, notes, status, created_at
        FROM payments
        {where_sql}
        ORDER BY payment_date DESC, id DESC
    """
    cur.execute(query, params)
    payments = [dict(
        id=r[0], payment_date=r[1], direction=r[2], party_type=r[3], party_name=r[4],
        amount=r[5], currency=r[6], payment_method=r[7], reference_no=r[8], notes=r[9],
        status=r[10], created_at=r[11]
    ) for r in cur.fetchall()]

    # Aggregates
    total_incoming = sum(p["amount"] for p in payments if p["direction"] == "INCOMING")
    total_outgoing = sum(p["amount"] for p in payments if p["direction"] == "OUTGOING")
    totals = {
    ...
    return render_template(
        "admin_payments.html", active_page="payments",
        dropdowns=dropdowns, filters=filters, totals=totals,
        payments=payments, today=str(date.today())
    )


@app.route("/admin/payments/save", methods=["POST"])
@admin_required
def admin_payments_save():
    direction = request.form.get("direction", "INCOMING").strip().upper()
    party_type = request.form.get("party_type", "CUSTOMER").strip().upper()
    party_name = request.form.get("party_name", "").strip()
    amount_val = safe_float(request.form.get("amount", "0"))
    currency = request.form.get("currency", "EUR").strip().upper()
    payment_method = request.form.get("payment_method", "BANK_TRANSFER").strip().upper()
    payment_date = request.form.get("payment_date", str(date.today())).strip()
    ...
    if amount_val <= 0:
        ...
        try:
            cur.execute("""
                INSERT INTO payments (payment_date, direction, party_type, party_name, amount, currency, payment_method, reference_no, notes, status, created_at)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'COMPLETED', ?)
            """, (payment_date, direction, party_type, party_name, amount_val, currency, payment_method, reference_no, notes, datetime.now().isoformat()))
            conn.commit()
            database.bump_app_revision()
            flash("Payment record saved successfully.", "success")
        except Exception as exc:
            conn.rollback()
            flash(f"Error saving payment record: {exc}", "error")
        finally:
            conn.close()

    redirect_args = {k: request.args.get(k, "").strip() for k in ("direction", "party_type", "party_name", "payment_method", "date_from", "date_to") if request.args.get(k, "").strip()}
    return redirect(url_for("admin_payments", **redirect_args))
...
def admin_payments_delete(payment_id):
    ...
    try:
        ...
        conn.rollback()
        flash(f"Error deleting payment record: {exc}", "error")
    finally:
        conn.close()
